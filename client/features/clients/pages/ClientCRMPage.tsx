/**
 * ClientCRMPage - Client Relationship Management Page
 *
 * CRM, Intake, and Client Portal Management with progressive loading.
 */
...
export const ClientCRMPage: React.FC = () => {
  ...
  const [selectedClientPortal, setSelectedClientPortal] =
  ...
  // Stable callback for showing intake modal
  ...
  // Stable callback for selecting client portal
  const handleSelectClientPortal = useLatestCallback(
    (client: Client | null) => {
      setSelectedClientPortal(client);
      if (client) {
        trackEvent('client_crm_portal_viewed', {
          clientId: client.id,
  ...
  // Stable callback for adding client with modal
  ...
  // Stable callback for 360 view action
  const handle360View = useLatestCallback((client: Client) => {
    trackEvent('client_crm_360_view_clicked', {
      clientId: client.id,
      clientName: client.name,
    });
  });
  ...
  return (
    <div className="space-y-6 relative animate-fade-in">
      ...
      {/* LazyHydration for progressive loading of client cards grid */}
      <LazyHydration
