/**
 * DashboardPage - Executive Dashboard Page Component
 *
...
import { PageHeader, StatCard, Card, Button, StatusCard } from '@/components/common';
          ...
          <HydrationBoundary id="dashboard-alerts" priority="normal" trigger="visible">
            <div className="space-y-4">
              {alerts.map((alert) => (
                <div
                  key={alert.id}
                  className={`flex items-start p-3 rounded-md transition-colors border-l-4 border-l-blue-500 bg-slate-50/50 ${
                    alert.caseId ? 'cursor-pointer hover:bg-blue-50' : ''
                  }`}
                  onClick={() => alert.caseId && handleCaseSelect(alert.caseId)}
                >
                  <div className="flex-1">
                    <p className={`text-sm font-medium ${
                      alert.caseId ? 'text-blue-700' : 'text-slate-900'
                    }`}>
                      {alert.message}
                    </p>
        ...
        <div className="bg-gradient-to-r from-purple-500 to-indigo-600 rounded-lg p-4 text-white text-sm">
          <div className="flex items-center gap-2">
            ...
            <span className="opacity-90">
              TanStack Query caching • useLatestCallback • useTrackEvent analytics
            </span>
          </div>
        </div>
      )}
