...
import { User, Lock, CheckCircle, Loader2, AlertCircle, Mail } from 'lucide-react';

interface AcceptInvitePageProps {
  token: string;
  onAccept: (data: AcceptInviteData) => Promise<boolean>;
  onLoadInvite?: (token: string) => Promise<InviteInfo | null>;
}
...
export const AcceptInvitePage: React.FC<AcceptInvitePageProps> = ({
  ...
  useEffect(() => {
    const loadInvite = async () => {
      if (onLoadInvite) {
        const info = await onLoadInvite(token);
        if (info) {
          setInviteInfo(info);
        } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const success = await onAccept({ name, password, token });
      if (success) {
        setIsSuccess(true);
      } else {
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50 flex items-center justify-center p-4">
        <div className="max-w-md w-full">
          <div className="bg-white rounded-2xl shadow-xl border border-slate-200 p-8 text-center">
            ...
            <h2 className="text-2xl font-bold text-slate-900 mb-2">Welcome to LexiFlow!</h2>
            <p className="text-slate-600 mb-6">
              Your account has been successfully created. You can now sign in and start using
              LexiFlow.
            </p>
            <a
              href="#login"
          ...
          <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-lg font-semibold text-blue-900">{inviteInfo.firmName}</p>
