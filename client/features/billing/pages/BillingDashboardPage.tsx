/**
 * BillingDashboardPage - Feature Module Page
 *
 * Financial performance dashboard with WIP vs Billed tracking,
 * realization breakdown visualization, and client portfolio management.
 *
 * ENZYME FEATURES:
 * - usePageView for page tracking
 * - HydrationBoundary for progressive chart hydration
 * - useTrackEvent for analytics
 * - useLatestCallback for stable handlers
 */
...
export const BillingDashboardPage: React.FC<BillingDashboardPageProps> = ({
  navigateTo,
}) => {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
          ...
          <div className="p-4 bg-slate-50/30">
            ...
            <div className="space-y-3">
              {clients.map((client) => (
                <div
                  key={`firm-${client.id}`}
                  className="flex items-center justify-between p-3 border border-slate-200 bg-white rounded-lg hover:border-blue-400 cursor-pointer"
                  onClick={handleClientClick}
                >
                  <div className="flex items-center space-x-3">
                    <Avatar
      ...
      <div className="flex items-center justify-center gap-2 py-3 px-4 bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-lg text-xs text-slate-600">
        ...
        <span>
          TanStack Query caching • Progressive Hydration • useLatestCallback •
