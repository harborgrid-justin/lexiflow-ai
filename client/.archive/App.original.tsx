...
/**
 * LexiFlow AI - Main Application Component
 * 
 * ROUTING:
 * This app uses Enzyme's hash-based router (useHashRouter) for client-side navigation.
 * - All routes are hash-based (e.g., /#workflows, /#cases, /#edit-case/123)
 * - The router provides analytics tracking for all navigation events
 * - Navigate using the `navigate(route)` function from useHashRouter
 * - Route params are automatically extracted (e.g., edit-case/123 -> params.id = "123")
 * 
 * @see /client/enzyme/hooks/useHashRouter.ts for router implementation
 */
  ...
  const renderContent = () => {
    ...
    try {
      switch (activeView) {
        ...
        case 'pacer-import': return <PacerImportPage onBack={() => navigate('cases')} onImportComplete={handleSelectCaseById} />;
        ...
        case 'billing': return <BillingDashboard navigateTo={navigate} />;
        case 'crm': return <ClientCRM />;
        case 'research': return <ResearchTool currentUser={user} />;
        case 'documents': return <DocumentManager currentUserRole={user.role as any} />;
        ...
        case 'workflow': // Alias for workflows
        ...
        default: {
          ...
          if (activeView.startsWith('edit-case')) {
            const caseId = params.id || activeView.replace('edit-case/', '');
            return <EditCaseDemo caseId={caseId} onBack={() => navigate('enzyme-demo')} />;
          }
          return <div className="flex justify-center items-center h-full text-slate-400">Under Construction</div>;
        }
      }
    } catch (error) {
      console.error('Error rendering view:', activeView, error);
      return (
        <div className="flex flex-col justify-center items-center h-full text-center p-8">
          <div className="text-red-500 mb-4">
            <ShieldAlert className="h-16 w-16 mx-auto mb-4" />
            <h2 className="text-xl font-semibold mb-2">Something went wrong</h2>
            <p className="text-slate-600 mb-4">Unable to load {activeView}</p>
          </div>
          <button
