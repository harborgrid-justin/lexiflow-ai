import { useBuffer, useLatestCallback } from '../index';
...
export const useAnalyticsBuffer = () => {
  ...
  const analyticsBuffer = useBuffer<AnalyticsEvent>({
    ...
    onFlush: async (events) => {
      ...
      try {
        ...
        await enzymeClient.post('/analytics/batch', { events });
      } catch (err) {
  ...
  const trackEvent = useLatestCallback((event: string, metadata?: Record<string, any>) => {
    analyticsBuffer.add({
      event,
      timestamp: Date.now(),
      metadata
