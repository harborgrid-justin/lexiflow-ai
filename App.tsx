...
const App: React.FC = () => {
  ...
  return (
    <div className="flex h-screen bg-slate-50 font-sans text-slate-900 overflow-hidden">
      ...
      <Sidebar activeView={selectedCase ? 'cases' : activeView} setActiveView={(v) => { setActiveView(v); setSelectedCase(null); setIsSidebarOpen(false); }} isOpen={isSidebarOpen} onClose={() => setIsSidebarOpen(false)} currentUser={currentUser} onSwitchUser={() => setCurrentUserIndex((prev) => (prev + 1) % MOCK_USERS.length)} />
      <div className="flex-1 flex flex-col md:ml-64 h-full transition-all w-full">
        <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-4 md:px-8 shadow-sm z-30 shrink-0">
          ...
          <div className="flex items-center space-x-6">
            ...
            <div className="flex items-center space-x-3 border-l pl-6"><div className="text-right hidden md:block"><p className="text-sm font-semibold">{currentUser.name}</p><p className="text-xs text-slate-500">{currentUser.role}</p></div><div className="h-9 w-9 bg-blue-100 rounded-full flex items-center justify-center text-blue-700 font-bold border border-blue-200"><UserIcon className="h-5 w-5" /></div></div>
          </div>
        </header>
